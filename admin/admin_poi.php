...
<a class="button" id="submit-btn" href="/addpoi" >Add</a>
        ...
        $sql = "SELECT novy.wiki,novy.shot,novy.admin,novy.id, novy.name, novy.dial, novy.info, novy.x,novy.y, novy.z, marker.icon, marker.name AS marker_name  FROM novy  JOIN marker ON novy.marker = marker.id ";
		...
		if (!(empty($san['shot'])||$san['shot']==="No Screenshot"))
	{
		...
		$shotres.='<img class="shot-img" src="https://novyapi.daktoinc.co.uk/api/staging/shot?id='.$row['id'].'">';
			$shotres.='</div>';
			$shotres.='</summary>';
			$shotres.='</details>';
