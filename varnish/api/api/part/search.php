...
$query = "SELECT n.id,n.wiki,n.admin, n.name, n.dial, n.x,n.y, n.z,n.shot, m.name AS marker_name, n.info
          FROM novy n
...
$response['data'] = [];
 ...
 if (!(empty($row['admin'])||$row['admin']==="???")){
		...
		{
		$response['data'][$row['id']]['screenshot'] = 'https://novyapi.daktoinc.co.uk/api/staging/shot?id='.$row['id'] ;
		  		} else{
